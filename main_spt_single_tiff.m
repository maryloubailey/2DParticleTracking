...
for z = 1:numFiles
    ...
    close all
    disp(files{z})
    [mov,numFrames,dirpath,frameRate,fname] = LoadMovie_Single(filepath{z},files{z});
    %     fits = []; %array containing number of fitted centroids for each frame
    ...
    if ~isempty(pk)
        ...
        pixelPos = [pixelPos; centroid intensity sigma error SNR ones(size(centroid,1),1)*z];
        ...
        if showplot == 1
            ...
            title([num2str(z) ' out of ' num2str(numFiles)]);
        end
    ...
    %add pixel position data to array after analyzing each frame
    %     pixelPosfinal = vertcat(pixelPosfinal,pixelPos);
    %     fitsFinal = vertcat(fitsFinal, fits);
...
%after going through all files, now write out final pixel position list

dlmwrite(fullfile(dirpath,'pixelPositions.txt'),pixelPosfinal,'delimiter','\t','newline','pc');
dlmwrite(fullfile(dirpath,'parameters.txt'),set_parameters,'delimiter','\t','newline','pc');
dlmwrite(fullfile(dirpath,'fits.txt'),fitsFinal,'delimiter','\t','newline','pc');
